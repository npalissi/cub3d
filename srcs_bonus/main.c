	...
	while (*game->map.full_map)
	{
		...
		if (!empty[0])
		{
			...
			if (!game->mini.skipped)
				break ;
			if (ft_strstr(str, game->color.cl) || ft_strstr(str, game->color.fl) || ft_strstr(str, game->texture.north) || ft_strstr(str, game->texture.south) || ft_strstr(str, game->texture.door) || 
				ft_strstr(str, game->texture.east) || ft_strstr(str, game->texture.west))
				continue;
			else
